...
public class BloomFilter<E> implements Serializable {
    ...
    private int expNumElements; // expected (maximum) number of elements to be added
    private int numAddElements; // number of elements actually added to the Bloom filter
    private int numhash; // number of hash functions
    ...
    BloomFilter(double falsePositiveProbability, int expectedNumberOfElements) {
        ...
        this.expNumElements = expectedNumberOfElements;
        this.numhash = (int)Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))); // numhash = ceil(-log_2(false prob.));
        this.bitsPerElement = Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2); // c = numhash / ln(2)
    ...
    private static int[] createHashes(byte[] data, int hashes) {
        ...
        while (numhash < hashes) {
            ...
            for (int i = 0; i < digest.length/4 && numhash < hashes; i++) {
                int h = 0;
    ...
    public int hashCode() {
        ...
        hash = 61 * hash + this.expNumElements;
        hash = 61 * hash + this.bitSetSize;
        hash = 61 * hash + this.numhash;
        return hash;
    }
...
//        return getFalsePositiveProbability(expNumElements);
//    }
//
//    private double getFalsePositiveProbability(double numberOfElements) {
//        // (1 - e^(-numhash * n / m)) ^ numhash
//        return Math.pow((1 - Math.exp(-numhash * (double) numberOfElements
//                / (double) bitSetSize)), numhash);
//
//    }
    ...
    private void add(byte[] bytes) {
        int[] hashes = createHashes(bytes, numhash);
        for (int hash : hashes)
